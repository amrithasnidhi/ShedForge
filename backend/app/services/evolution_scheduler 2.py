...
class EvolutionaryScheduler:
                        ...
                        for _ in range(blocks_needed):
                            def collect_options(
                                *,
                                ...
                                per_day_limit_base = max(1, option_limit // active_day_count)
                                ...
                                for day, slots in self.day_slots.items():
                                    per_day_limit = per_day_limit_base
                                    per_start_limit = max(
                                        4,
                                        min(12, (per_day_limit // max(1, len(slots))) + 2),
                                    )
                                    diversity_target = len(faculty_option_order) * min(4, max(1, len(request_room_candidates)))
                                    per_start_limit = max(
                                        per_start_limit,
                                        min(diversity_target, 96),
                                    )
                                    per_day_limit = max(
                                        per_day_limit,
                                        min(option_limit, per_start_limit * min(4, max(1, len(slots)))),
                                    )
                                    for start_index in range(0, len(slots) - block_size + 1):
        ...
        *,
        ...
        def ordered_candidates(req_index: int) -> list[int]:
            ...
            if fixed_faculty_id is not None:
                ...
                if planned_matches:
                    all_candidate_indices = planned_matches
            ...
            if not feasible_indices and len(all_candidate_indices) < len(req.options):
                if planned_faculty_id is not None and not req.is_lab:
                    return []
    ...
    def _constructive_individual(
        ...
        *,
        ...
        if strict_dead_end:
            return self._constructive_individual_strict(
